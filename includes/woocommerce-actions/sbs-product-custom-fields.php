...
function sbs_add_custom_general_fields_save( $post_id ) {
  ...
  $merch_cred_field = $_POST['merch_credit'];
  if ( isset( $merch_cred_field ) )
    update_post_meta( $post_id, 'merch_credit', $merch_cred_field );

  $required_product_field = isset( $_POST['required_product'] ) ? 'yes' : 'no';
  update_post_meta( $post_id, 'required_product', $required_product_field );

  $autoadd_product_field = isset( $_POST['autoadd_product'] ) ? 'yes' : 'no';
  update_post_meta( $post_id, 'autoadd_product', $autoadd_product_field );
